...
Specializations. `/api/specializations` (list/candidates/assign, all `requireRole(ADMIN)` at the
route) plus `specializationService#assertSpecializationAccess` at the service layer — mentors have
no read or write surface, even for their own assigned interns. Not workspace-scoped (intern
domain, same exception as Recommendations/Project above). The intern's own
...
endpoint is unaffected (stays intern-writable).
