...
- Narrow. `npm test` in `server/` (Jest) covers pure helpers (`helpers/*.test.js`) plus three
  ...
  `frontend/` (vitest) covers a handful of pure helpers under `src/helpers/*.test.js`, plus the
  pure cart functions exported from `src/hooks/useStagedPicks.js`, and nothing else — no component
  is rendered in a test anywhere. There is no integration or E2E suite.
