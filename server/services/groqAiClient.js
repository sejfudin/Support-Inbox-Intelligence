...
async function requestGroqOutputText({ prompt }) {
  ...
  try {
    ...
    if (!response.ok) {
      throw createAiServiceError(`AI provider request failed with status ${response.status}.`, 503);
    }
    ...
    throw createAiServiceError('AI suggestion is currently unavailable. Please try again.', 503);
  }
}
