...
export const CreateSprintModal = ({ open, onOpenChange, workspaceId, nextSprintName }) => {
  ...
  useEffect(() => {
    if (open) reset(buildDefaults(nextSprintName));
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4">
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              />
              {errors.start && (
                <p className="text-xs text-[hsl(var(--tone-danger-fg))]">{errors.start.message}</p>
              )}
            </div>
          ...
          <DialogFooter>
            ...
            <Button type="submit" disabled={mutation.isPending} data-test="sprint-create-save">
